...
describe("CharacterNotes", () => {
  describe("view mode", () => {
    it("shows default text for all empty fields", () => {
      render(<CharacterNotes character={emptyCharacter} onUpdate={jest.fn()} />)
    ...
    it("shows populated field values", () => {
      render(<CharacterNotes character={populatedCharacter} onUpdate={jest.fn()} />)
  ...
  describe("edit mode", () => {
    ...
    it.each([
      ...
      render(<CharacterNotes character={emptyCharacter} onUpdate={onUpdate} />)
      enterEditMode()
      fireEvent.change(screen.getByLabelText(new RegExp(labelPattern, "i")), {
        target: { value },
      })
      fireEvent.click(screen.getByRole("button", { name: /save changes/i }))
